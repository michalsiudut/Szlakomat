...
internal sealed class LockProductHandler
    : IRequestHandler<LockProduct, Result<string, InventoryLockId>>
{
    ...
    public Task<Result<string, InventoryLockId>> Handle(
        LockProduct command,
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(command.ProductId))
        {
            return Task.FromResult(
                Result<string, InventoryLockId>.FailureOf("ProductId is required"));
        }
        ...
        return Task.FromResult(result.Map(_ => lockId));
    }
}
